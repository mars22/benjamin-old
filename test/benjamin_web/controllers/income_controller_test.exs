defmodule BenjaminWeb.IncomeControllerTest do
  ...
  describe "new income" do
    test "renders form", %{conn: conn, budget: budget} do
      conn = get conn, Routes.budget_income_path(conn, :new, budget.id)
  ...
  describe "create income" do
    test "redirects to budget show when data is valid", %{conn: conn, budget: budget} do
      conn = post conn, Routes.budget_income_path(conn, :create, budget.id), income: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.budget_path(conn, :show, id, tab: :incomes)
    ...
    test "renders errors when data is invalid", %{conn: conn, budget: budget} do
      conn = post conn, Routes.budget_income_path(conn, :create, budget.id), income: @invalid_attrs
      assert html_response(conn, 200) =~ "New Income"
    end
  end
  ...
  describe "edit income" do
    test "renders form for editing chosen income", %{conn: conn, user: user, budget: budget} do
      ...
      conn = get conn, Routes.budget_income_path(conn, :edit, budget.id, income.id)
  ...
  describe "update income" do
    ...
    test "redirects when data is valid", %{conn: conn, budget: budget, income: income} do
      conn = put conn, Routes.budget_income_path(conn, :update, budget, income), income: @update_attrs
      assert redirected_to(conn) == Routes.budget_path(conn, :show, budget, tab: :incomes)
    ...
    test "renders errors when data is invalid", %{conn: conn, budget: budget, income: income} do
      conn = put conn, Routes.budget_income_path(conn, :update, budget, income), income: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Income"
    end
  end
  ...
  describe "delete budget" do
    ...
    test "deletes chosen income", %{conn: conn, budget: budget, income: income} do
      conn = delete conn, Routes.budget_income_path(conn, :delete, budget, income)
      assert redirected_to(conn) == Routes.budget_path(conn, :show, budget, tab: :incomes)
      assert_error_sent 404, fn ->
        get conn, Routes.budget_income_path(conn, :edit, budget, income)
      end
