...
api.env.roledefs = {"PROD": ["aws"]}

api.env.project_src = "/home/ubuntu/app/benjamin/"
api.env.git_url = "<<GIT_REPO_URL>>"
api.env.prompts = {"Are you sure you want to do this?": "yes"}


@api.roles("PROD")
def update():
    with contextlib.nested(api.cd("%(project_src)s" % api.env)):
        api.run("git pull")
        api.run("MIX_ENV=prod mix deps.get")
        with contextlib.nested(api.cd("%(project_src)s/assets" % api.env)):
